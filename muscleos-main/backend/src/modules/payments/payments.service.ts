...
export class PaymentsService {
  ...
  async initiate(dto: CreatePaymentDto, gymId: string, collectedById: string) {
    ...
    // complete on the strength of a client-supplied UTR alone â€” see
    ...
    // bank statement. UPI must NOT auto-complete â€” a member/staff-typed
    ...
    if (dto.gateway === PaymentGateway.RAZORPAY) {
      ...
      // Cash / bank-transfer only â€” generate the receipt right away since
   ...
   * must become PAYABLE, or the member has no way to ever pay it â€” the
   ...
   *  by another payment in this gym â€” a member (accidentally or otherwise)
   ...
   *  together (e.g. gateway=CASH with method=ONLINE) â€” without this,
