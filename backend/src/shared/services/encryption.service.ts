...
export class EncryptionService {
  ...
  constructor(private readonly configService: ConfigService) {
    const secret = this.configService.get('app.jwtSecret') || 'default-jwt-secret-key-change-in-production';
    this.legacyKey = crypto.scryptSync(secret, 'salt', 32);

    const customKey = process.env.ENCRYPTION_KEY || this.configService.get('ENCRYPTION_KEY');
    if (customKey) {
      this.key = customKey.length === 64
        ? Buffer.from(customKey, 'hex')
        : crypto.scryptSync(customKey, 'muscleos-salt', 32);
    } else {
      this.key = this.legacyKey;
    }
  }
  ...
  decrypt(encryptedData: string): string {
    ...
    try {
      ...
      if (this.key !== this.legacyKey) {
        const decipher = crypto.createDecipheriv(this.algorithm, this.legacyKey, iv);
        decipher.setAuthTag(authTag);
        let decrypted = decipher.update(encrypted, 'hex', 'utf8');
        decrypted += decipher.final('utf8');
        return decrypted;
