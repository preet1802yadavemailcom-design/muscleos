...
export class GymsService {
  ...
  async dashboardStats(gymId: string) {
    ...
    const [
      ...
      todayCheckIns, currentlyCheckedIn, totalBatches, activeBatches,
      revenueAgg, revenueTodayAgg, pendingPayments,
    ] = await Promise.all([
      ...
      this.prisma.attendance.count({ where: { gymId, checkOutAt: null } }),
    ...
    return {
      members: { total: totalMembers, active: activeMembers, inactive: inactiveMembers, expired: expiredMemberships },
      attendance: { checkInsToday: todayCheckIns, currentlyInGym: currentlyCheckedIn },
      batches: { total: totalBatches, active: activeBatches },
      revenue: { total: revenueAgg._sum.total ?? 0, today: revenueTodayAgg._sum.total ?? 0, pendingPayments },
    };
  }
